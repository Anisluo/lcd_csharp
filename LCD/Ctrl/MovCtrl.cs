...
namespace LCD.Ctrl
{
    ...
    public class MovCtrl
    {
        ...
        public void UpdataIO(ref int x, ref int y, ref int z, ref int u, ref int v, ref int ball,ref int TestSignal1, ref int TestSignal2)
        {

            if (Project.cfg.ax_x.IsEnable)
                x = MPC08EDLL.checkin_bit(board_cnt1, 1);

            if (Project.cfg.ax_y.IsEnable)
                y = MPC08EDLL.checkin_bit(board_cnt1, 3 );

            if (Project.cfg.ax_z.IsEnable)
                z = MPC08EDLL.checkin_bit(board_cnt1, 2);

            if (Project.cfg.ax_u.IsEnable)
                u = MPC08EDLL.checkin_bit(board_cnt1, 5);

            if (Project.cfg.ax_v.IsEnable)
                v = MPC08EDLL.checkin_bit(board_cnt1, 4);

            if (Project.cfg.ax_ball.IsEnable)
                ball = MPC08EDLL.checkin_bit(board_cnt1, 0);
