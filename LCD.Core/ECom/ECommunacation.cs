...
namespace VisionCore
{
    ...
    public class ECom
    {
        public bool IsOpen { get { return IsConnected; } set { if (value) Connect(); else DisConnect(); } }// 是否开始连接 或监听
        ...
        public string m_ReadData { get; set; } = "";//收到的数据
        ...
        public bool IsAutoNewLine { get; set; } = true;// 自动添加新行
        ...
        internal MySerialPort GetSerialPort()
        {
            return m_MySerialPort;
        }
            ...
            lock (this)
            {
                ...
                switch (CommunicationModel)
                {
                    ...
                    case CommunicationModel.COM:
                        if (m_MySerialPort != null)
                        {
                            flag = (bool)m_MySerialPort?.WriteData(str, IsSendByHex, IsAutoNewLine);
                        }
                        break;
                    default:
                        break;
